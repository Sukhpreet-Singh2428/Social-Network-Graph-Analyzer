...
import { Users2, ArrowRight, Layers, ShieldAlert, CheckCircle2 } from 'lucide-react';
...
export const CommunitiesPage: React.FC = () => {
  ...
  const largestCommunitySize = Math.max(...communities.map(c => c.memberCount), 0);
  const isolatedUserCount = communities.filter(c => c.memberCount === 1).length;
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div>
        <h1 className="text-2xl font-extrabold text-zinc-100 flex items-center gap-2.5 tracking-tight">
          ...
          Connected Components & Community Clusters
        </h1>
        <p className="text-xs text-zinc-400 mt-1">
          Algorithmic BFS connected component detection. Clusters are dynamically calculated from backend graph topology.
        </p>
      </div>

      {/* Summary Stats Strip */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-4 rounded-xl bg-[#18181b] border border-white/10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-extrabold text-zinc-100">{totalCommunities} Component(s)</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-[#18181b] border border-white/10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-extrabold text-zinc-100">{largestCommunitySize} User(s)</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-[#18181b] border border-white/10 flex items-center gap-3">
          ...
          <div>
            <span className="text-[9px] font-mono text-zinc-400 uppercase font-bold">Isolated Users (N=1)</span>
            <p className="text-xl font-extrabold text-zinc-300">{isolatedUserCount} User(s)</p>
          </div>
        </div>
      </div>
      ...
      {communities.length === 0 ? (
        <div className="p-12 text-center rounded-xl bg-[#18181b] border border-white/10 text-zinc-400 text-xs">
          No communities or users available in the backend graph store.
        </div>
      ) : (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-400">ID: #{u.id} • {u.role}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => {
